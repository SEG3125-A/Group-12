...
export default function OptionCard({ imageSrc, cardTitle, onClick, isActive, description, altText, destinationLink = '' }) {
  return (
    <Link to={destinationLink} style={{ textDecoration: 'none', pointerEvents: destinationLink ? 'auto' : 'none' }}>
      <div className={`tertiary-bg border position-relative ${isActive ? 'active-card' : ''}`} style={{ maxWidth: '395px', borderRadius: '29px' }} onClick={onClick}>
